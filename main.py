...
to_replace = {'%all_users%': lambda: db_api.count.users(),
              ...
              '%money_for_refs%': lambda: list(db_api.sumof.transactions(row='count', type='pay_new_ref'))[0]['sum(count)'],
...
def send_welcome(message):
    ...
    if len(message.text.split(' ')) > 1 and message.text.split(' ')[1] != str(message.from_user.id):
        send_message(message, {'text':'''Благодарю, что Вы воспользовались приглашением [пользователя!](tg://user?id={})'''.format(message.text.split(' ')[1]),'markup':messages.start['markup']}, parse_mode="Markdown")
        ...
        tb.send_message(message.text.split(' ')[1], """😃 Вам начислен *{}* руб за  приглашенного вами друга 😎""".format(settings.new_ref_cost), parse_mode="Markdown")
...
def nuka(message):
    ...
    if text in ['⛔️ Отмена']:
        ...
        return
    if text == '◀️ Вернуться' or text == '🔙 Начало':
        return send_message(message, {'text': 'Вы в главном меню.', 'markup': messages.start['markup']})
    ...
    # Смотрим посты
        ...
        mes_id = send_message(message, {'text': m, 'markup':[['⛔️ Отмена']]}, parse_mode='Markdown')
...
💸Доход с рефералов 1 Уровня - 10%
💸Доход с рефералов 2 Уровня - 4%'''.format(referals, refs2nd, bot_info.username, user['user_id']),
...
💸Доход с рефералов 1 Уровня - 10%
💸Доход с рефералов 2 Уровня - 4%'''.format(user['referal'], referals, refs2nd, bot_info.username, user['user_id'], bot_info.username, user['user_id']),
                    'markup': messages.start['markup']
                }
                try:
...
💸Доход с рефералов 1 Уровня - 10%
💸Доход с рефералов 2 Уровня - 4%'''.format(
...
💸 Выведено всего: *{}*p
    '''.format(message.from_user.id, len(usr_chn), view_count, view_bal, post_bal, round(ref_pay, 2), round(view_bal + post_bal + round(ref_pay, 2), 2),
        ...
        send_message(message, msg, parse_mode="Markdown")
        return

    elif text == '💵 Баланс':
        answr = {'text': '''💰 Ваш общий баланс: {}р
        💵 Баланс: {}р
        👥 Реферальный баланс: {}р'''.format(round(user['balance'], 2), round(user['balance'], 2), round(user['ref_balance'], 2)),
        ...
        obj['text'] = obj['text'].replace('%max_money%', str(round(user['balance'])))
    ...
    if text == 'Яндекс':
        ...
        obj['text'] = obj['text'].replace('%max_money%', str(round(user['balance'], 2)))
        send_message(message, obj)
    ...
    if text == 'Другой способ':
        return send_message(message, {'text': 'Если вы хотите пополнить баланс другим способом - напишите администратору - [админ](tg://user?id=557668057)', 'markup': messages.start['markup']}, parse_mode="Markdown")
    if text == '🔖 Мои заказы':
...
🔧🔨Чтобы добавить ваш канал для раскрутки - действуйте по инструкции, указанной выше.''', 'markup': messages.for_subs['markup']})
    ...
    if text == 'Яндекс или Банковская карта':
        send_message(message, messages.balance['ya'], parse_mode="Markdown")
        send_message(message, {'text':'''{}'''.format(message.from_user.id)})
        return
        ##################### Вывод меню
    ...
    #     tb.send_message(chat_id=message.chat.id, text='Каналы в базе: 1 cписок', reply_markup=answ.inline_channels(page_n=1))
    #     # tb.send_message(chat_id=message.chat.id, text='Каналы в базе: 2 cписок',
    #     #                 reply_markup=answ.inline_channels(page_n=2))
    #     # tb.send_message(chat_id=message.chat.id, text='Каналы в базе: 3 cписок',
    #     #                 reply_markup=answ.inline_channels(page_n=3))
    #     # tb.send_message(chat_id=message.chat.id, text='Каналы в базе: 4 cписок',
    #     #                 reply_markup=answ.inline_channels(page_n=4))
    #     # tb.send_message(chat_id=message.chat.id, text='Каналы в базе: 5 cписок',
    #     #                 reply_markup=answ.inline_channels(page_n=5))
    #     return
    #
    #
    # if text == 'Посты' and user.get('menu', '') in ['admin-moderation']:
    #     tb.send_message(chat_id=message.chat.id, text='Посты:: 1 список', reply_markup=answ.inline_posts(page_n=1))
    #     tb.send_message(chat_id=message.chat.id, text='Посты: 2 список', reply_markup=answ.inline_posts(page_n=2))
    #     tb.send_message(chat_id=message.chat.id, text='Посты: 3 список', reply_markup=answ.inline_posts(page_n=3))
    #     tb.send_message(chat_id=message.chat.id, text='Посты: 4 список', reply_markup=answ.inline_posts(page_n=4))
    #     tb.send_message(chat_id=message.chat.id, text='Посты: 5 список', reply_markup=answ.inline_posts(page_n=5))
    #     return
...
# print(s)
# requests.get('https://stats.pessom.ru/join_bot', params=s, verify=False)
# print(tb.set_webhook(
#     url=settings.WEBHOOK_URL_BASE + settings.WEBHOOK_URL_PATH,
#     allowed_updates=['update_id', 'message', 'edited_message', 'channel_post', 'edited_channel_post', 'inline_query', 'chosen_inline_result', 'callback_query',
...
while True:
    try:
        threading.Thread(target=answ.check_qiwi,kwargs={'send_message':send_message}).start()
        tb.polling(none_stop=True, timeout=99)
