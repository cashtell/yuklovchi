...
🔝Ваш заказ выполнен, все заказанные люди подписались на канал {}!'''.format(p['channel_name']))
            ...
            if channel_name not in user_channels:
                ...
                mes_id = self.tb.send_message(chat_id=message.chat.id,text="""Подпишись на этот канал:
1️⃣ Перейдите на канал 👇, подпишитесь ✔️ и пролистайте ленту вверх 🔝👁 (*5-10* постов).
2️⃣ Возвращайтесь⚡️сюда, чтобы получить 💰 вознаграждение.""",parse_mode="Markdown",reply_markup=self.gen_inl([[{'text':'1️⃣  Перейти к каналу','url':'https://t.me/{}'.format(p['channel_name'].replace('@',''))}],[{'text':'2️⃣  Проверить подписку','data':'chck-public-{}'.format(p['channel_name'].replace('@',''))}]]))
    ...
    def check_sub(self,channel,user,send_message,message):
        ...
        try:
            ...
            if user_c.status =='member' and channel not in user_channels:
                ...
                self.tb.edit_message_text(chat_id=message.chat.id,message_id=message.message_id,text='Вы все еще не подписаны!',reply_markup=self.gen_inl([[{'text':'Перейти к каналу','url':'https://t.me/{}'.format(channel.replace('@',''))}],[{'text':'2️⃣  Проверить подписку','data':'chck-public-{}'.format(channel)}]]))
                return False
        except:
            user_channels.append(channel)
            self.db.insert.users(user_id=user['user_id'], channels=str(user_channels))
